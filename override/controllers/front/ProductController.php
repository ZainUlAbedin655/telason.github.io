...
class ProductController extends ProductPresentingFrontControllerCore{
    ...
    public function displayAjaxRefresh(){
        ...
        $isPreview          = ('1' === Tools::getValue('preview'));
        $product_new        = new Product($product['id_product']);
        $final_combination  = array();
       
        if(isset($id_product_attribute) && !empty($id_product_attribute)){
            $final_combinations = json_decode($this->getCustomField($product['id_product'],$id_product_attribute), true);
            if($final_combinations){
                foreach($final_combinations as $key => $comb_value){
                    if(($comb_value['combination'] == $id_product_attribute) && ($comb_value['currency'] == $currency_id || empty($comb_value['currency']))){
                        $final_combination[] = $final_combinations[$key];
                    }
                }
            }
        }else{
            $final_combinations = json_decode($this->getCustomField($product['id_product']), true);
        ...
        array(
            ...
            'product_discountss'    => $this->render('../../../modules/pricerule/views/price_table',
                array(
                'id_product_attribute'      => $id_product_attribute,
                'product_price'             => json_decode(Product::getPriceStatic($product['id_product'],false,$id_product_attribute)),
            ...
            'id_product_attribute'      => $product['id_product_attribute'],
            'id_product_attribute_id1'  => $id_product_attribute,
    ...
    protected function assignAttributesGroups($product_for_template = null,$id_product_attribute_id = null,$stringDev = ''){
        $colors = array();
        $groups = array();
        ...
        if (is_array($attributes_groups) && $attributes_groups) {
            ...
            foreach($groups as $group_key => $group) {
                ...
                if($count > 1) {
                    ...
                    $query_id_attributes = 'SELECT `id_attribute` FROM `'._DB_PREFIX_.'product_attribute_combination` pac2
                    ...
                    $id_attributes = Db::getInstance()->executeS($query_id_attributes);
                    foreach($id_attributes as $k => $row) {
                        $id_attributes[$k] = (int)$row['id_attribute'];
                    }
                    foreach($group['attributes'] as $key => $attribute) {
                        if(!in_array((int)$key, $id_attributes)) {
                            //echo 'unsetting id '.$key.' <br>';
                            unset($groups[$group_key]['attributes'][$key]);
                            unset($groups[$group_key]['attributes_quantity'][$key]);
                        }
                    }
                }
                $index = 0;
            ...
            $this->context->smarty->assign(array(
                ...
                'id_product_attribute'  => $id_product_attribute_id,
                'stringDev'             => $stringDev
            ));
        } else {
            $this->context->smarty->assign(array(
        ...
        if($useGroups === true) {
            ...
            if(!empty($groups)){
                if(isset($this->product->id) && !empty($this->product->id)){
                    try{
                    $requestedIdProductAttribute = (int) Product::getIdProductAttributesByIdAttributes($this->product->id,$groups);
                    }catch(Exception $e){
                    	// try again with three attributes
                    	unset($groups['6']); 
                    	$requestedIdProductAttribute = (int) Product::getIdProductAttributesByIdAttributes($this->product->id,$groups);
                        //echo json_encode(array('error' => 'Product Variation Not Found'));
                        //exit;
                    }
                    /*        getIdProductAttributeByIdAttributes && getIdProductAttributesByIdAttributes*/
                }
            }
        }

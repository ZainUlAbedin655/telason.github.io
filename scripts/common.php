<?php
function sheetsrun($excel,$sheets, $access_bnot = 6,$connector){
    ob_implicit_flush(true);
    ob_start();
    // echo '<pre>';
    //     print_r($sheets);
    //     echo $currentSheet = current($sheets);
    // echo '<pre>';
    // exit;
    $currentSheet = 0;
    if(isset($_GET) && !empty($_GET)){
        if(isset($_GET['currentSheet']) && !empty($_GET['currentSheet'])){
            $currentSheet = $_GET['currentSheet'];
        }
    }
    foreach($sheets as $sheetkey => $sheet){ 
        if($sheetkey == $currentSheet || $currentSheet == 0){
            if($currentSheet == 0){
                $currentSheet = $sheetkey;
            }
            if(isset($sheets[$currentSheet]) && !empty($sheets[$currentSheet])){
                $bnot = 0;
                ...
                foreach($sheet as $referStatic => $productIDStatic){
                    if($productIDStatic > 0) {
                        ...
                        foreach($data as $key => $item){
                            ...
                            if($bnot == 0){
                                ...
                                if(strlen($item[7]) == 1){
                                    ...
                                    if($item[7] == 'B'){
                                        ...
                                        $addString = ' : Band Strap | Termination Band | Coiled';
                                    }
                                    if($item[7] == 'S'){
                                ...
                                $accessory      = $addNone.$item[7].$addString;
                            ...
                            $finish_codes   = $item[6-$bnot];
                            /*echo '<pre>';
                                                    print_r($finish_codes);
                                                     //exit;
                                                    print_r($item);
                                                    print_r($item[7-$bnot]);
                                                    print_r($bnot);
                            ...
                            $combination[$key]['finish_codes']  = $finish_codes;
                            
                            $finish_codes = explode(',', $combination[$key]['finish_codes']);
                            $finish_codes[] = 'S';
                            $finish_codes[] = 'CD';
                            ...
                            if($ref == $referStatic && $product_id == $productIDStatic){
                                   ...
                                   //$combination = getCombinationByProductId($product_id,$shell,$entry,$code,$accessory);
                                        $code_attribute_id = getAttributeIdByValueFinish($code.$codeDesc);
                                            ...
                                            try{
                                                ...
                                                $combination[$key][$code]['full_code'] = $code.$codeDesc;
                                                $combination[$key][$code]['code_attribute_id'] = $code_attribute_id;  
                                                $combination[$key][$code]['shell']       = $shell;
                                                $combination[$key][$code]['shell_attribute_id']    = $shell_id;
                                                $combination[$key][$code]['entry']       = $entry;
                                                $combination[$key][$code]['entry_attribute_id']    = $entry_id;
                                                $combination[$key][$code]['finish_code'] = $code;
                                                $combination[$key][$code]['entry_attribute_id']    = $entry_id;
                                                if(!empty($accessory)){
                                                    $combination[$key][$code]['accessory']   = $accessory;
                                                    $combination[$key][$code]['accesory_attribute_id']    = $accesory_id;
                                                }
                                                $combination[$key][$code]['attributeIds']    = $attributeIds;
                                                ...
                                                foreach($qty_heads as $key2 => $qty_head){
                                                    ...
                                                    //$combination[$key][$code]['qty'][$qty_head]['price']['usd'] = $priceLess * 0.85;
                                                    $combination[$key][$code]['qty'][$qty_head] = (($realPrice - $codeSImpact) * $impactMultipler) + $codeSImpact;
                                                    //$combination[$key][$code]['qty'][$qty_head]['price']['gbp'] = $item[$key2+10];
                                                }
                     
                                                $combination[$key][$code]['qty1Price'] = $qty1Price;
                                            ...
                                            $combination[$key][$code]['combination_status']   = false; 
                                            if($combinationId == 0 || $combinationId == false || empty($combinationId)){
                                                ...
                                                $combination[$key][$code]['combination_status']   = 'created'; 
                                            }else{
                                                //update combination price 
                                                $combination[$key][$code]['combination_id_before']   = $combinationId;

                                                $combinationId = updateCombination($product,$attributeIds,$ProductPrice,$qty1Price,$combinationId);
                                                $combination[$key][$code]['combination_status']   = 'update';
                                            }
                                            $combination[$key][$code]['combination_id']   =  $combinationId;

                                            $combination[$key][$code]['product_id']       = $product_id;
                                            $combination[$key][$code]['impactMultipler']  = $impactMultipler;
                                ...
                                unset($combination[$key]['finish_codes']);
                            }else{
                                echo 'Static Ids do not match Ids in code'.$ref.' == '.$referStatic .'&&'. $product_id .' == '. $productIDStatic;
                                echo ' Ref is combine '.$item[1].' & '.$item[2].' & '.$item[3].' that all';
                                exit;
                            }
                        }
                        $counter = 0 ;
                        ...
                        if(!empty($combination)){
                            ...
                            foreach($combination as $key2 => $finish_codes){
                                if($key2 == $current || $current == 0){
                                     $key2++;
                                    $current = $key2;
                                    foreach($finish_codes as $code => $combinationData){
                                        foreach($combinationData['qty'] as $qty => $price){
                                                       $post_update = array();
                                                     ...
                                                     //insert 
                                                      ...
                                                      if($specific_price_id && $specific_price_id > 0){
                                                        ...
                                                        echo "<pre>";
                                                        $counter++;
                                                        echo 'combinations post_update '.$counter;
                                                        print_r($rules_inner);
                                                        echo "</pre>";
                                                        ob_flush();
                                    ...
                                    ob_end_flush();
                                    sleep(1);
                                    $url = 'https://telason.com/scripts/connector_'.$connector.'.php?current='.$current.'&currentSheet='.$currentSheet;
...
function getQtyDeliveryTime($qty, $product_id){
    ...
    $Products3weeks = array(96,97,98,99,100,101,102,103,104);
    if($qty <= 50 && in_array($product_id, $Products3weeks)){
        $DeliveryTime = '1 Week';
    }else{

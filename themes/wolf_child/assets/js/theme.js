...
$(document).on('mouseover', '.sub-menu ul[data-depth="3"] > li', function(e){
    $(this).parents('.sub-menu ul[data-depth="3"]').find('li').removeClass('active');
    $(this).addClass('active');
});
$(document).on('mouseover', '.sub-menu ul[data-depth="4"] > li', function(e){
    $(this).parents('.sub-menu ul[data-depth="4"]').find('li').removeClass('active1');
    $(this).addClass('active1');
});
$(document).on('mouseout', '.sub-menu ul[data-depth="3"] > li', function(e){
    $(this).removeClass('active');
});
$(document).on('mouseout', 'ul[data-depth="4"] > li', function(e){
    $(this).removeClass('active1');
});
$('.top-menu .navbar-toggler.collapse-icons').click(function(){
    var target = $(this).data('target');
    $(target).parent().toggleClass('collapse-in');
    $(target).parent().find('.sub-menu-collapse').toggleClass('in');
    $(target).parent().find('.sub-menu-collapse .sub-menu-collapse').removeClass('in');
    $(target).parent().find('.sub-menu-collapse li').removeClass('collapse-in');
});

/*$(window).resize(function(){
    if ($(window).width() <= 767){  
jQuery('ul.clearfix').each(function(){
var list=jQuery(this),
    select=jQuery(document.createElement('select')).insertBefore(jQuery(this).hide()).change(function(){
  window.open(jQuery(this).val(),'_newtab')
});

   ...
   <div class="products tab-container">
      ...
      {assign var='sliderFor' value=5} <!-- Define Number of product for SLIDER -->
		...
		{foreach from=$products item="product"}
			<li class="{if $slider == 1 && $no_prod >= $sliderFor}item{else}product_item col-xs-12 col-sm-6 col-md-4 col-lg-3 col-xl-4{/if}">				
		...
		<a class="all-product-link float-xs-left pull-md-right h4" href="{$allBestSellers}">
			{l s='View More Products' mod='pst_bestsellers'}
		</a>
		{/if}
		{if $slider == 1 && $no_prod >= $sliderFor}
	  </div>

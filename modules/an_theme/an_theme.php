...
class an_theme extends Module
{
    ...
    protected function exportConfiguration()
    {
        ...
        return @file_put_contents($configurationSource, $configurationJson);
    }
        ...
        foreach ($keys as $key) {
            if (in_array($key, $lang)) {
                foreach (Language::getLanguages(1, 0, 1) as $id_lang) {
                    $config[$key][$id_lang] = $this->getParamValue($key, $id_lang);
                }
            } else {
                $config[$key] = $this->getParamValue($key);
            }
        }
    ...
    {
        $file = (string)self::getCSSFile();
        @unlink(_PS_MODULE_DIR_.$this->name.'/views/css/'.$file);
        return $file;
    }

    protected static function getCSSFile()
    {
        return Configuration::get(self::CSS_FILE);
    }

    protected static function updateCSSFile($file)
    {
        return Configuration::updateValue(self::CSS_FILE, $file);
    }

    protected static function generateCSSName($name)
    {
        return ($name !== false ? md5($name.self::SALT) : md5("theme")).'.css';
    }
    ...
    protected function getParamValue($key, $id_lang = null)
    {
        $input = $this->getInputFactory()->create($key);
        $defaultValue = $input->getData('default', '');
        
        return Configuration::get(self::PREFIX.$key, $id_lang, null, null, $defaultValue);
    }
     ...
     * @return mixed
     */
    public function getParam($key, $value = null, $id_lang = null)
    { 
        if ($value === null) {
            return $this->getParamValue($key, $id_lang);
        }
    ...
    protected function updateParamValue($key, $value)
    {
        $input = $this->getInputFactory()->create($key);
    ...
    protected function imageRenderer($field_id, $option, $input)
    {
        ...
        $image = _PS_MODULE_DIR_ . $this->name . '/views/img/' . $this->getParam($field_id);
        $image_url = false;
        $thumb_size = false;
    ...
    protected function generateCSS()
    {
        ...
        return @file_put_contents(_PS_MODULE_DIR_.$this->name.'/views/css/'.$export, $this->display($this->name, 'css_generator.tpl')) ? $this->updateCSSFile($export) : false;
    }
        ...
        foreach ($fields as $field_key => $field) {
            ...
            foreach ($field['options'] as $option_key => $option) {
                ...
                if (isset($option['lang']) && $option['lang'] === true) {
                    ...
                    foreach (Language::getLanguages(1, 0, 1) as $id_lang) {
                        $helper->fields_value[$field_id][$id_lang] = $this->getParam($field_id, null, (int)$id_lang);
                    }
                } else {
                    $helper->fields_value[$field_id] = $this->getParam($field_id);
                }
        ...
        $fields_form[] = array(
            'form' => array(
                ...
                'buttons' => array_merge(array(
                    ...
                    )), (Tools::getIsset('dev') ? array(
                    array(
                        'type' => 'submit',
                        'name' => self::EXPORT_CONFIGURATION,
                        'title' => $this->l('Export system configuration')
                    )) : array())
                )
            )
        );
        ...
        foreach ($this->_fields as $field_key => $field) {
            foreach ($field['options'] as $option_key => $option) {
                if (isset($option['default'])) {
                    $this->getParam($field_key.'_'.$option_key, $option['default']);
                }
            }
        }
    ...
    protected function deleteImage()
    {
        ...
        $path = realpath(_PS_MODULE_DIR_.$this->name.'/views/img/'.$this->getParam($image));
    ...
    {
        $this->setupConfiguration();
        if (Tools::getIsset(self::EXPORT_CONFIGURATION)) {
                    ...
                    if (isset($_FILES[$field_id])) {
                        if (!$_FILES[$field_id]['error']) {
                            if ($result = $this->uploadImage($field_id)) {
                                @unlink(_PS_MODULE_DIR_ .$this->name.'/views/img/'.$this->getParam($field_id));
                                $this->getParam($field_id, $result);
                            } else {
                                $this->errors[] = $this->l('Error during file uploading');
                            }
                        }
                    } else {
                        $this->getParam($field_id, $value);
                    }
                }
            }
        }

...
class PSTLeftBanner extends ObjectModel
{
	public $title;
	public $url;
	public $image;
	...
	public static $definition = array(
		'table' => 'pstleftbanner_slides',
		'primary' => 'id_pstleftbanner_slides',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'pstleftbanner` (`id_shop`, `id_pstleftbanner_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'pstleftbanner`
			WHERE `id_pstleftbanner_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_pstleftbanner_slides` = hs.`id_pstleftbanner_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_pstleftbanner_slides` as id_slide
			FROM `'._DB_PREFIX_.'pstleftbanner_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'pstleftbanner` hs ON (hss.`id_pstleftbanner_slides` = hs.`id_pstleftbanner_slides`)

<?php
//not working //working override main
...
class ProductController extends ProductPresentingFrontControllerCore
{
        ...
        if (!Validate::isLoadedObject($this->product)) {
            ...
            if ((!$this->product->isAssociatedToShop() || !$this->product->active) && !$isPreview) {
                if ($isAssociatedToProduct) {
                    ...
                    if(!$this->product->id_type_redirected ||(in_array($this->product->redirect_type, array('301-product', '302-product')) && $this->product->id_type_redirected == $this->product->id)
                    ){
    ...
    public function displayAjaxRefresh(){
        ...
        $combinations =  $product_new->getAttributeCombinations($this->context->language->id);
        $attributeGroups = $product_new->getAttributesGroups($this->context->language->id);
        $combArray = [];
        $combArrayIds = [];
        foreach($combinations as $k => $v ){   
            ...
            $final_price = json_decode($this->getFieldValue($product['id_product'],"price"));
                ...
                if($final_combinations){
                    foreach($final_combinations as $key => $comb_value){
                        if(
                            ...
                            && ($comb_value['specific_price_id'] > 0))
                        ){
    ...
    private function getIdProductAttribute($useGroups = false)
    {
        ...
        if ($useGroups === true) {
            ...
            if (!empty($groups)) {
            PrestaShopLogger::addLog("ProductController: getIdProductAttribute, product_id:".$this->product->id, '1');
    ...
    public function getTemplateVarProduct()
    {
        ...
        $product['new'] = (int) $this->product->new;
            PrestaShopLogger::addLog("ProductController: getTemplateVarProduct", '1');
        
        $product['id_product_attribute'] = $this->getIdProductAttribute(false);

...
class Pst_HomeSlide extends ObjectModel
{
	public $title;
	public $description;
	public $url;
	...
	public static $definition = array(
		'table' => 'psthomeslider_slides',
		'primary' => 'id_psthomeslider_slides',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'psthomeslider` (`id_shop`, `id_psthomeslider_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'psthomeslider`
			WHERE `id_psthomeslider_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_psthomeslider_slides` = hs.`id_psthomeslider_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_psthomeslider_slides` as id_slide
			FROM `'._DB_PREFIX_.'psthomeslider_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'psthomeslider` hs ON (hss.`id_psthomeslider_slides` = hs.`id_psthomeslider_slides`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new Pst_HomeSlide($row['id_slide']);

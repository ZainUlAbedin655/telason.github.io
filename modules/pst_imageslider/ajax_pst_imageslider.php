...
include_once('pst_imageslider.php');

$home_slider = new Pst_ImageSlider();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'psthomeslider_slides` SET `position` = '.(int)$position.'
			WHERE `id_psthomeslider_slides` = '.(int)$id_slide
		);

...
class Pst_Footerlink extends Module implements WidgetInterface
{
	/* @var boolean error */
	...
	public function __construct()
	{
		...
		$this->displayName = $this->trans('Footer Link block', array(), 'Modules.Footerlink');
		$this->description = $this->trans('Add custom link in footer.', array(), 'Modules.Footerlink');
		
		//$this->confirmUninstall = $this->trans('Are you sure you want to delete all your links?', array(), 'Modules.Footerlink');
		
		$this->ps_versions_compliancy = array('min' => '1.7', 'max' => _PS_VERSION_);
	
		$this->templateFile = 'module:pst_footerlink/views/templates/hook/pst_footerlink.tpl';
	}
	...
	public function installDB()
    {
		...
		$return &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'pstfooterlink` (
			`id_pstfooterlink` int(10) NOT NULL AUTO_INCREMENT,
			`url` varchar(254) NOT NULL,
			`new_window` TINYINT(1) NOT NULL,
			PRIMARY KEY(`id_pstfooterlink`))
			ENGINE='._MYSQL_ENGINE_.' default CHARSET=utf8'
		);
			
		$return &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'pstfooterlink_shop` (
			`id_pstfooterlink` int(10) NOT NULL AUTO_INCREMENT,
			`id_shop` int(10) NOT NULL,
			PRIMARY KEY(`id_pstfooterlink`, `id_shop`))
			ENGINE='._MYSQL_ENGINE_.' default CHARSET=utf8'
		);
			
		$return &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'pstfooterlink_lang` (
			`id_pstfooterlink` int(10) NOT NULL,
			`id_lang` int(10) NOT NULL,
			`text` varchar(62) NOT NULL,
			PRIMARY KEY(`id_pstfooterlink`, `id_lang`))
			ENGINE='._MYSQL_ENGINE_.' default CHARSET=utf8'
		);
	...
	public function uninstallDB($drop_table = true) {
		$ret = true;
        if ($drop_table) {
            $ret &=  Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'pstfooterlink`') && 
				Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'pstfooterlink_shop`') &&
				Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'pstfooterlink_lang`');
	...
	public function getLinkById($id)
	{
		...
		{
			$link = [];
			$sql = 'SELECT b.`id_pstfooterlink`, b.`url`, b.`new_window` FROM `'._DB_PREFIX_.'pstfooterlink` b WHERE b.id_pstfooterlink='.(int)$id;

			if (!$results = Db::getInstance()->getRow($sql))
				return false;

			$link['id_pstfooterlink'] = (int)$results['id_pstfooterlink'];
			$link['url'] = $results['url'];
			$link['newWindow'] = $results['new_window'];

			$results = Db::getInstance()->executeS('SELECT `id_lang`, `text` FROM '._DB_PREFIX_.'pstfooterlink_lang WHERE `id_pstfooterlink`='.(int)$link['id_pstfooterlink']);
	...
	public function getLinks()
	{
		...
		$sql = 'SELECT b.`id_pstfooterlink`, b.`url`, b.`new_window`
				FROM `'._DB_PREFIX_.'pstfooterlink` b';
		if (Shop::isFeatureActive() && Shop::getContext() != Shop::CONTEXT_ALL)
			$sql .= ' JOIN `'._DB_PREFIX_.'pstfooterlink_shop` bs ON b.`id_pstfooterlink` = bs.`id_pstfooterlink` AND bs.`id_shop` IN ('.implode(', ', Shop::getContextListShopID()).') ';
		$sql .= (int)Configuration::get('CT_BLOCKLINK_ORDERWAY') == 1 ? ' ORDER BY `id_pstfooterlink` DESC' : '';
		...
		{
			$result[$i]['id'] = $link['id_pstfooterlink'];
			...
			if (!$texts = Db::getInstance()->executeS('
					SELECT `id_lang`, `text`
					FROM '._DB_PREFIX_.'pstfooterlink_lang
					WHERE `id_pstfooterlink`='.(int)$link['id_pstfooterlink']
				)
			)
				return false;
	...
	public function addLink()
	{
		...
		if ((int)Tools::getValue('id_pstfooterlink') > 0)
		{
			$id_link = (int)Tools::getValue('id_pstfooterlink');
			if (!Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'pstfooterlink SET `url` = \''.pSQL(Tools::getValue('url')).'\', `new_window` = '.pSQL((int)Tools::getValue('newWindow')).' WHERE `id_pstfooterlink` = '.(int)$id_link))
				return false;
			if (!Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'pstfooterlink_lang WHERE `id_pstfooterlink` = '.(int)$id_link))
				return false;

			foreach ($languages as $language)
				if (Tools::getValue('text_'.$language['id_lang']) != false )
				{
					if (!Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'pstfooterlink_lang (`id_pstfooterlink`, `id_lang`, `text`) VALUES ('.(int)$id_link.', '.(int)($language['id_lang']).', \''.pSQL(Tools::getValue('text_'.$language['id_lang'])).'\')'))
						return false;
				}
				else
					if (!Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'pstfooterlink_lang VALUES ('.(int)$id_link.', '.$language['id_lang'].', \''.pSQL(Tools::getValue('text_'.$id_lang_default)).'\')'))
						return false;
		}
		else
		{
			if (!Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'pstfooterlink (`id_pstfooterlink`, `url`, `new_window`)
			...
			foreach ($languages as $language)
				if (Tools::getValue('text_'.$language['id_lang']) != false)
				{
					if (!Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'pstfooterlink_lang (`id_pstfooterlink`, `id_lang`, `text`)
																VALUES ('.(int)$id_link.', '.(int)$language['id_lang'].', \''.pSQL(Tools::getValue('text_'.$language['id_lang'])).'\')')
					)
						return false;
				}
				else
					if (!Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'pstfooterlink_lang (`id_pstfooterlink`, `id_lang`, `text`) VALUES ('.(int)$id_link.', '.(int)($language['id_lang']).', \''.pSQL(Tools::getValue('text_'.$id_lang_default)).'\')'))
						return false;
		}

		Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'pstfooterlink_shop WHERE id_pstfooterlink='.(int)$id_link);

		$shops = Shop::getShops(true, null, true);
		if (!Shop::isFeatureActive() || (Shop::isFeatureActive() && count($shops) == 1))
		{
			Db::getInstance()->insert('pstfooterlink_shop', array(
				'id_pstfooterlink' => (int)$id_link,
	...
	{
		return (Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'pstfooterlink WHERE `id_pstfooterlink` = '.(int)Tools::getValue('id')) &&
			Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'pstfooterlink_shop WHERE `id_pstfooterlink` = '.(int)Tools::getValue('id')) &&
			Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'pstfooterlink_lang WHERE `id_pstfooterlink` = '.(int)Tools::getValue('id')));
	}
	...
	public function getContent()
	{
		...
		// Add a link
		if (Tools::isSubmit('submitLinkAdd') /*|| Tools::isSubmit('updatepst_footerlink')*/)
		{
		...
		// Delete a link
		elseif (Tools::isSubmit('deletepst_footerlink') && Tools::getValue('id'))
		{
	...
	public function renderForm()
	{
		...
		$helper->identifier = 'id_pstfooterlink';
	...
	{
		$fields_values = array(
			'id_pstfooterlink' => Tools::getValue('id_pstfooterlink'),
		...
		if (Tools::getIsset('updatepst_footerlink') && (int)Tools::getValue('id') > 0)
    ...
    {
        $this->context->controller->registerStylesheet('modules-pst_footerlink', 'modules/'.$this->name.'/views/css/pst_footerlink.css', ['media' => 'all', 'priority' => 150]);        
    }
	
	public function renderWidget($hookName = null, array $configuration = [])
    {
        if (!$this->isCached($this->templateFile, $this->getCacheId('pst_footerlink'))) {
            $this->smarty->assign($this->getWidgetVariables($hookName, $configuration));
        }

        return $this->fetch($this->templateFile, $this->getCacheId('pst_footerlink'));
    }

...
class ProductCore extends ObjectModel
{
    ...
    */
    ...
    public static function getIdProductAttributeByIdAttributes($idProduct, $idAttributes, $findBest = false)
    {
        ...
        $idProductAttribute =  Db::getInstance()->getValue($query3);
        ...
        if ($idProductAttribute === false && $findBest) {
            ...
            $query2 = 'SELECT 
                ...
                ORDER BY 
                    g.`position` ASC';
            $result = Db::getInstance()->executeS($query2);
            ...
            while ($idProductAttribute === false && count($orderred) > 0) {
                ...
                $idProductAttribute =  Db::getInstance()->getValue($query);
            }
        }

        if (empty($idProductAttribute)) {
            PrestaShopLogger::addLog("classes/Prodcut.php: getIdProductAttributeByIdAttributes, product_id:", '1');
            PrestaShopLogger::addLog("classes/Prodcut.php: ".$query2.", product_id:", '1');
            PrestaShopLogger::addLog("classes/Prodcut.php: ".$query3."getIdProductAttributeByIdAttributes, product_id:", '1');
            PrestaShopLogger::addLog("classes/Prodcut.php: ".$query."getIdProductAttributeByIdAttributes, product_id:", '1');
            throw new PrestaShopObjectNotFoundException('Can not retrieve the id_product_attribute '.$query.' &= '.$query2.' &= '.$query3);
        }
